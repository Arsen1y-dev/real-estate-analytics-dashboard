...
**Фильтры** — цена, площадь, комнаты, год постройки, расстояние до центра, тип дома, этаж (исключить первый/последний), планировки. Диапазоны задаются полями «От» / «До». Блок **«Дополнительные фильтры»** — произвольные условия по любой колонке датасета (как в настройках графиков): для чисел — ≥, ≤, между; для текста — равно, содержит.
...
**Таблица** — настраиваемые колонки, сортировка, ссылка на объявление, reverse geocoding для пустых адресов (ручной запуск/остановка, прогресс, кэш).
...
Для строк без нормального адреса, но с координатами, таблица может подтянуть текстовый адрес через API.
...
Геокодинг **не** стартует сам — кнопки «Запустить геокодинг» / «Остановить геокодинг». При открытии таблицы snapshot по ключам подтягивает уже сохранённые адреса.
